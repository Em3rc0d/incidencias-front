...
export function VehiclePage() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:8080/api/vehiculos/${editingId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <section className="max-w-6xl mx-auto mt-12 px-6 space-y-10">
      ...
      <div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
          {vehiculos.map((v) => (
            <div key={v.id} className="bg-white shadow-md rounded-lg p-5 border border-gray-200 space-y-2">
              ...
              <details className="mt-2">
                ...
                <div className="mt-2 text-sm text-gray-700 space-y-1">
                  ...
                  <p><strong>ID Empresa:</strong> {v.empresaId}</p>
                </div>
              </details>

              <div className="flex gap-2 pt-2">
                <Button variant="outline" onClick={() => handleEdit(v)}>Editar</Button>
                <Button variant="destructive" onClick={() => handleDelete(v.id)}>Eliminar</Button>
              </div>
            </div>
          ))}
        </div>
